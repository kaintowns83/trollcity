...
export default function BroadcasterApplicationPage() {
  ...
  const { data: application } = useQuery({
    ...
    queryFn: async () => {
      const apps = await base44.entities.BroadcasterApplication.filter({ user_id: user.id });
      return apps[0] || null;
    },
    enabled: !!user?.id,
  });
  ...
  const submitApplicationMutation = useMutation({
    mutationFn: async () => {
      ...
      // Upload ID document
      const { file_url } = await base44.integrations.Core.UploadFile({ file: idFile });
...
- notes: string (any concerns or observations)`,
      ...
      const app = await base44.entities.BroadcasterApplication.create({
        user_id: user.id,
        ...
        ai_verification_result: JSON.stringify(aiVerification),
        status: shouldApprove ? "approved" : "pending",
        approved_date: shouldApprove ? new Date().toISOString() : null,
        approved_by: shouldApprove ? "AI Auto-Approval" : null
      });
      ...
      if (shouldApprove) {
        ...
        await base44.auth.updateMe({
          ...
          broadcaster_application_id: app.id
        });
      }
  ...
  const meetsLevelRequirement = userLevel >= 20; // Changed from 15 to 20

  // Calculate XP needed - Harder progression
  ...
  const xpNeededForLevel20 = getTotalXPNeeded(20); // Calculate total XP to reach Level 20
  const currentXP = user?.experience || 0; // Use user.experience
          ...
          <Card className="bg-gradient-to-r from-orange-500/20 to-red-500/20 border-orange-500 p-6 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-bold text-white mb-2">🔒 Level 20 Required</h3>
                <p className="text-orange-200 mb-3">
                  You must reach <strong>Level 20</strong> before you can apply to receive payouts.
                </p>
                <div className="bg-[#0a0a0f] rounded-lg p-4">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-gray-300">Level 20 Requires:</span>
                    <span className="text-yellow-400 font-bold">{xpNeededForLevel20.toLocaleString()} XP</span>
                  </div>
                  <div className="bg-[#1a1a24] rounded-full h-3 overflow-hidden">
                    <div 
                      className="bg-gradient-to-r from-purple-500 to-pink-500 h-full transition-all duration-500"
                      style={{ 
                        width: `${Math.min((currentXP / xpNeededForLevel20) * 100, 100)}%` 
                      }}
                    />
                  </div>
          ...
          {!meetsLevelRequirement && (
            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 mb-6">
              <p className="text-red-300 text-sm flex items-center gap-2">
                ...
                You cannot submit an application until you reach Level 20
              </p>
            </div>
          )}
